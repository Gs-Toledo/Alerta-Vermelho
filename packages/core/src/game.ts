...
export default class Game {
  ...
  public iniciarJogo(nomesJogadores: string[]): void {
    ...
    this.estado.jogadores = nomesJogadores.map((nome, index) => {
      ...
      const localizacaoInicial =
        localizacoes[Math.floor(Math.random() * localizacoes.length)].id;
      return {
        id: `jogador-${index + 1}`,
        nome,
        cargo,
    ...
    this.logger.addLog(
      `Jogadores: ${this.estado.jogadores
        .map((j) => `${j.nome} (${j.cargo})`)
        .join(", ")}`,
      0
    );

    // Inicializa baralhos
    this.estado.baralhoJogador = this.cardManager.shuffle(
      localizacoes.map((loc) => loc.id)
    );
    this.estado.baralhoQueimada = this.cardManager.shuffle(
      localizacoes.map((loc) => loc.id)
    );
    ...
    this.logger.addLog(
      "Jogo iniciado! Que a batalha contra as queimadas comece!",
  ...
  public realizarAcao(
    ...
    if (this.estado.jogoAcabou) {
      this.logger.addLog(
        "O jogo já acabou. Nenhuma ação pode ser realizada.",
        this.estado.turnoAtual
      );
      return false;
    }
    ...
    if (!jogador) {
      this.logger.addLog(
        `Erro: Jogador com ID ${jogadorId} não encontrado.`,
        this.estado.turnoAtual
      );
      return false;
    }
    ...
    if (jogador.acoesRestantes <= 0) {
      this.logger.addLog(
        `${jogador.nome} não tem mais ações restantes neste turno.`,
        this.estado.turnoAtual
      );
      return false;
    }
    ...
    switch (acao) {
      ...
      case TipoAcao.PLANEJAMENTO:
        // Define a habilidade dependendo do cargo atual
        switch (jogador.cargo) {
          case CargoJogador.MINISTRO_MEIO_AMBIENTE:
            acaoBemSucedida = this._habilidadeMinistro(jogador);
            break;
          case CargoJogador.GOVERNADOR:
            const localizacaoAlvoId = args[0];
            acaoBemSucedida = this._habilidadeGovernador(
              jogador,
              localizacaoAlvoId
            );
            break;
          case CargoJogador.PARLAMENTAR:
            acaoBemSucedida = this._habilidadeParlamentar(jogador);
            break;
          default:
            this.logger.addLog(
              `Cargo desconhecido (${jogador.cargo}) não possui ação de planejamento.`,
              this.estado.turnoAtual
            );
    ...
    if (acaoBemSucedida) {
      ...
      this.logger.addLog(
        `${jogador.nome} realizou a ação: ${acao}. Ações restantes: ${jogador.acoesRestantes}`,
        this.estado.turnoAtual
      );
  ...
  public avancarTurno(): void {
    if (this.estado.jogoAcabou) {
      this.logger.addLog(
        "O jogo já acabou. Não é possível avançar o turno.",
        this.estado.turnoAtual
      );
      return;
    }
    ...
    this.estado.turnoAtual++;
    this.logger.addLog(
      `--- Iniciando Turno ${this.estado.turnoAtual} ---`,
      this.estado.turnoAtual
    );
    ...
    if (!this.estado.jogoAcabou) {
      this.logger.addLog(
        `Trilha de Queimada: ${this.estado.trilhaQueimada}, Trilha de Flora: ${this.estado.trilhaFlora}`,
        this.estado.turnoAtual
      );
    }
  }
  ...
  private _distribuirCartasIniciaisJogadores(): void {
    this.estado.jogadores.forEach((jogador) => {
      ...
      this.logger.addLog(
        `${jogador.nome} recebeu suas cartas iniciais.`,
        this.estado.turnoAtual
      );
    });
  }
  ...
  private _comprarCartaJogador(jogador: Jogador): void {
    if (this.estado.baralhoJogador.length === 0) {
      this.logger.addLog(
        "Baralho de Jogadores vazio, reembaralhando descarte.",
        this.estado.turnoAtual
      );
      if (this.estado.descarteJogador.length === 0) {
        // Se o descarte também estiver vazio, o baralho acabou completamente.
        this.logger.addLog(
          "O baralho e descarte de jogadores estão vazios. Jogo pode acabar.",
          this.estado.turnoAtual
        );
    ...
    if (cartaId) {
      ...
      this.logger.addLog(
        `${jogador.nome} comprou a carta: ${cartaId}.`,
        this.estado.turnoAtual
      );
    }
  }
  ...
  private _habilidadeMinistro(jogador: Jogador): boolean {
    const estadoQueimada = this.estado.estadosQueimadas.find(
      (eq) => eq.id === jogador.localizacaoAtual
    );
    if (estadoQueimada) {
      estadoQueimada.protecaoAmbiental = Math.min(
        100,
        estadoQueimada.protecaoAmbiental + 30
      ); // Aumento de 30%
      this.logger.addLog(
        `Ministro fortaleceu a proteção em ${estadoQueimada.id} para ${estadoQueimada.protecaoAmbiental}%.`,
        this.estado.turnoAtual
      );
      return true;
    }
  ...
  private _habilidadeGovernador(
    jogador: Jogador,
    localizacaoAlvoId: string
  ): boolean {
    if (!localizacaoAlvoId) {
      this.logger.addLog(
        "Governador tentou usar a habilidade sem especificar um alvo.",
        this.estado.turnoAtual
      );
      return false;
    }

    const localizacaoAtual = this.mapManager.findLocationById(
      jogador.localizacaoAtual
    );
    const localizacaoAlvo = this.mapManager.findLocationById(localizacaoAlvoId);

    if (
      !localizacaoAtual ||
      !localizacaoAlvo ||
      localizacaoAtual.regiao !== localizacaoAlvo.regiao
    ) {
      this.logger.addLog(
        `Alvo inválido para o Governador. ${localizacaoAlvoId} não está na região ${localizacaoAtual?.regiao}.`,
        this.estado.turnoAtual
      );
      return false;
    }

    const estadoQueimadaAlvo = this.estado.estadosQueimadas.find(
      (eq) => eq.id === localizacaoAlvoId
    );
    if (estadoQueimadaAlvo && estadoQueimadaAlvo.nivelQueimada > 0) {
      estadoQueimadaAlvo.nivelQueimada--;
      this.logger.addLog(
        `Governador mobilizou forças para combater queimada em ${localizacaoAlvo.estado}. Nível atual: ${estadoQueimadaAlvo.nivelQueimada}.`,
        this.estado.turnoAtual
      );
      return true;
    } else {
      this.logger.addLog(
        `Governador não pôde agir em ${localizacaoAlvo.estado} (sem queimadas ativas).`,
        this.estado.turnoAtual
      );
      return false;
    }
  ...
  private _habilidadeParlamentar(jogador: Jogador): boolean {
    this.logger.addLog(
      "Parlamentar articulou apoio e comprou uma carta extra.",
      this.estado.turnoAtual
    );
    this._comprarCartaJogador(jogador);
    return true;
  ...
  private _moverJogador(jogador: Jogador, destinoId: string): boolean {
    ...
    if (!localizacaoAtual || !destino) {
      this.logger.addLog(
        `Erro de movimento: Localização atual (${jogador.localizacaoAtual}) ou destino (${destinoId}) inválido para ${jogador.nome}.`,
        this.estado.turnoAtual
      );
      return false;
    }
    ...
    if (this.mapManager.areAdjacent(jogador.localizacaoAtual, destinoId)) {
      ...
      this.logger.addLog(
        `${jogador.nome} moveu-se para ${destino.estado}.`,
        this.estado.turnoAtual
      );
      return true;
    } else {
      this.logger.addLog(
        `Erro de movimento: ${jogador.nome} não pode se mover diretamente de ${localizacaoAtual.estado} para ${destino.estado}.`,
        this.estado.turnoAtual
      );
      return false;
    }
  ...
  private _combaterQueimada(jogador: Jogador): boolean {
    ...
    if (!estadoQueimada || estadoQueimada.nivelQueimada === 0) {
      this.logger.addLog(
        `${jogador.nome} tentou combater queimada em ${jogador.localizacaoAtual}, mas não há queimada ativa.`,
        this.estado.turnoAtual
      );
      return false;
    }
    ...
    estadoQueimada.nivelQueimada = Math.max(
      0,
      estadoQueimada.nivelQueimada - 1
    );
    this.logger.addLog(
      `${jogador.nome} combateu queimada em ${estadoQueimada.id}. Nível atual: ${estadoQueimada.nivelQueimada}.`,
      this.estado.turnoAtual
    );
    return true;
  }
  ...
  private _cooperar(
    ...
    if (!jogador2 || jogador1.localizacaoAtual !== jogador2.localizacaoAtual) {
      this.logger.addLog(
        `Erro de cooperação: ${jogador1.nome} e ${
          jogador2?.nome || "outro jogador"
        } não estão na mesma localização ou o jogador não existe.`,
        this.estado.turnoAtual
      );
      return false;
    }
    ...
    if (cartaIndex === -1) {
      this.logger.addLog(
        `Erro de cooperação: ${jogador1.nome} não possui a carta ${cartaId}.`,
        this.estado.turnoAtual
      );
      return false;
    }
    ...
    this.logger.addLog(
      `${jogador1.nome} cooperou com ${jogador2.nome}, trocando a carta ${cartaId}.`,
      this.estado.turnoAtual
    );
    return true;
  }
  ...
  private _realizarPlanejamento(jogador: Jogador): boolean {
    this.logger.addLog(
      `${jogador.nome} realizou uma ação de planejamento. (Funcionalidade a ser expandida: Ex: Comprar mais cartas, fortificar localização)`,
      this.estado.turnoAtual
    );
  ...
  private _processarFaseQueimada(): void {
    this.logger.addLog(
      "Iniciando Fase de Queimadas...",
      this.estado.turnoAtual
    );
    const numCartasQueimada = GAME_CONSTANTS.NUM_CARTAS_QUEIMADA_POR_FASE(
      this.estado.trilhaQueimada
    );
    ...
    this.estado.estadosQueimadas.forEach((estado) => {
      ...
      const chanceAumento =
        GAME_CONSTANTS.AUMENTO_CHANCE_QUEIMADA_NATURAL *
        (1 - estado.protecaoAmbiental / 100);
      if (estado.nivelQueimada > 0 && Math.random() < chanceAumento) {
        if (estado.nivelQueimada < GAME_CONSTANTS.MAX_NIVEL_QUEIMADA) {
          ...
          this.logger.addLog(
            `Queimada em ${estado.id} aumentou para nível ${estado.nivelQueimada} naturalmente.`,
            this.estado.turnoAtual
          );
        } else {
    ...
    const totalNivelQueimada = this.estado.estadosQueimadas.reduce(
      (sum, eq) => sum + eq.nivelQueimada,
      0
    );
    this.estado.trilhaQueimada = Math.min(
      GAME_CONSTANTS.MAX_TRILHA_QUEIMADA,
      totalNivelQueimada * 5
    ); // Cada nível de queimada contribui para a trilha global
    this.logger.addLog(
      `Fase de Queimadas concluída. Trilha de Queimada atual: ${this.estado.trilhaQueimada}`,
      this.estado.turnoAtual
    );
    ...
    if (this.estado.trilhaQueimada >= GAME_CONSTANTS.MAX_TRILHA_QUEIMADA) {
      ...
      this.logger.addLog(
        "O nível de queimadas atingiu o máximo! O jogo terminou em DERROTA.",
        this.estado.turnoAtual
      );
    }
  }
  ...
  private _sortearNovaQueimada(): void {
    if (this.estado.baralhoQueimada.length === 0) {
      this.logger.addLog(
        "Baralho de Queimadas vazio, reembaralhando descarte.",
        this.estado.turnoAtual
      );
      this.estado.baralhoQueimada = this.cardManager.shuffle(
    ...
    if (estadoId) {
      ...
      if (estadoQueimada) {
        if (estadoQueimada.nivelQueimada < GAME_CONSTANTS.MAX_NIVEL_QUEIMADA) {
          ...
          this.logger.addLog(
            `Nova queimada sorteada em ${estadoQueimada.id}. Nível: ${estadoQueimada.nivelQueimada}.`,
            this.estado.turnoAtual
          );
        } else {
  ...
  private _propagarQueimada(
    estadoId: string,
    estadosVisitados: Set<string> = new Set()
  ): void {
    ...
    this.estado.trilhaQueimada = Math.min(
      GAME_CONSTANTS.MAX_TRILHA_QUEIMADA,
      this.estado.trilhaQueimada + GAME_CONSTANTS.AUMENTO_TRILHA_QUEIMADA_SURTO
    );

    localizacao.adjacentes.forEach((adjacenteId) => {
      const estadoQueimadaAdjacente = this.estado.estadosQueimadas.find(
        (eq) => eq.id === adjacenteId
      );
      if (estadoQueimadaAdjacente) {
        if (
          estadoQueimadaAdjacente.nivelQueimada <
          GAME_CONSTANTS.MAX_NIVEL_QUEIMADA
        ) {
          estadoQueimadaAdjacente.nivelQueimada++;
          this.logger.addLog(
            `Queimada se espalhou para ${estadoQueimadaAdjacente.id}. Nível: ${estadoQueimadaAdjacente.nivelQueimada}.`,
            this.estado.turnoAtual
          );
        } else {
  ...
  private _processarFaseEvento(): void {
    ...
    if (this.estado.baralhoEvento.length === 0) {
      this.logger.addLog(
        "Baralho de Eventos vazio. Não há novos eventos.",
        this.estado.turnoAtual
      );
      return;
    }
  ...
  private _atualizarTrilhaFlora(): void {
    if (
      this.estado.trilhaQueimada <
      GAME_CONSTANTS.NIVEL_QUEIMADA_PARA_RECUPERACAO_FLORA
    ) {
      this.estado.trilhaFlora = Math.min(
        GAME_CONSTANTS.MAX_TRILHA_FLORA,
        this.estado.trilhaFlora + GAME_CONSTANTS.RECUPERACAO_FLORA_TURNO_PADRAO
      );
      this.logger.addLog(
        "A flora está se recuperando devido ao controle das queimadas.",
        this.estado.turnoAtual
      );
    } else {
      this.estado.trilhaFlora = Math.max(
        GAME_CONSTANTS.MIN_TRILHA_FLORA,
        this.estado.trilhaFlora - GAME_CONSTANTS.DEGRADACAO_FLORA_TURNO_PADRAO
      );
      this.logger.addLog(
        "A flora está degradando devido ao alto nível de queimadas.",
        this.estado.turnoAtual
      );
    }
  }
  ...
  private _verificarCondicoesVitoria(): void {
    ...
    const floraRecuperada =
      this.estado.trilhaFlora >= GAME_CONSTANTS.VITORIA_FLORA_LIMIAR;

    if (todasQueimadasControladas && floraRecuperada) {
      this._finalizarJogo(true);
      this.logger.addLog(
        "Parabéns! Todas as queimadas controladas e a flora recuperada. VITÓRIA!",
        this.estado.turnoAtual
      );
      return;
    }
    ...
    if (this.estado.turnoAtual >= GAME_CONSTANTS.MAX_TURNOS) {
      this._finalizarJogo(false);
      this.logger.addLog(
        `Limite de ${GAME_CONSTANTS.MAX_TURNOS} turnos atingido. O jogo terminou em DERROTA.`,
        this.estado.turnoAtual
      );
      return;
    }
    ...
    if (this.estado.trilhaQueimada >= GAME_CONSTANTS.MAX_TRILHA_QUEIMADA) {
      this._finalizarJogo(false);
      this.logger.addLog(
        "A trilha de queimada atingiu o máximo! O jogo terminou em DERROTA.",
        this.estado.turnoAtual
      );
      return;
    }
    ...
    if (this.estado.trilhaFlora <= GAME_CONSTANTS.MIN_TRILHA_FLORA) {
      this._finalizarJogo(false);
      this.logger.addLog(
        "A flora foi completamente devastada! O jogo terminou em DERROTA.",
        this.estado.turnoAtual
      );
      return;
    }
    ...
    if (
      this.estado.baralhoJogador.length === 0 &&
      this.estado.descarteJogador.length === 0 &&
      this.estado.turnoAtual > 1
    ) {
      this._finalizarJogo(false);
      this.logger.addLog(
        "O baralho de cartas dos jogadores acabou. Não há mais cartas para comprar. O jogo terminou em DERROTA.",
        this.estado.turnoAtual
      );
      return;
    }
  ...
  private _finalizarJogo(vitoria: boolean): void {
    ...
    this.logger.addLog(
      `FIM DE JOGO! Resultado: ${vitoria ? "VITÓRIA" : "DERROTA"}`,
      this.estado.turnoAtual
    );
  }
}
